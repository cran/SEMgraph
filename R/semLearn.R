...
#'
#' @return A list of 3 igraph objects and 1 data matrix:
#' \itemize{
#' \item "dag", the output DAG (BAP = dag + guu),
...
#' input graph and (iii) the overall Shipley's' P-value.
#'
#' @author Mario Grassi \email{mario.grassi@unipv.it}
#'
#' @references
...
SEMdag<- function(graph, data, LO="TO", beta=0, lambdas=NA, penalty=TRUE, verbose=FALSE, ...)
{
	...
	}else{ dag<- ig } # gplot(dag); V(dag)$name
	#X<- scale(data[,V(dag)$name]) #colnames(X)
